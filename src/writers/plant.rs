...
impl WriteableEntity for Plant {
    fn write_entity(&self, addr: usize, game: &crate::game::Popcapgame) {
        game.write_at(addr, PlantOffset::DisplayPosX, self.display_pos_x);
        game.write_at(addr, PlantOffset::DisplayPosY, self.display_pos_y);
        game.write_at(addr, PlantOffset::Row, self.row);
        game.write_at::<u32>(addr, PlantOffset::PlantType, self.plant_type.into());
        game.write_at(addr, PlantOffset::Column, self.column);
        game.write_at(addr, PlantOffset::PlantState, self.plant_state);
        game.write_at(addr, PlantOffset::Health, self.health);
